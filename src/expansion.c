...
//expansion matrix a by the specific dimension
//the resulting matrix C is put into array byRow
void expansion(double *a, int *dimA,  int *dimT,double *c,int *byDim)
{
	int sizeA;
	int i,j;
	int by=byDim[0]-1;
	...
	for(i=dimTotal-2;i>=0;i--){
		if(i!=by-1){
			...
			mappingC[i]=mappingC[i+1];
		}
	}
	if(by>0) mappingC[by]=mappingC[0]*dimA[0];
